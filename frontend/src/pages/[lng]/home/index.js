...
function Index({ selfUser, centralNoticeList }) {

  const i18n = useI18n()
  const router = useRouter();

  return (
    <Layout title={`${i18n.t('pages.home.index.title')} - ${process.env.NEXT_PUBLIC_APPLICATION_NAME}`}>
      <Section
        title={i18n.t('pages.home.index.title')}
        titlePrefix={<IconButton><HomeIcon /></IconButton>}
        titleSuffix={[
          <IconButton onClick={() => router.push(`/${lng}/central-notices/list/`)}>
            <AnnouncementIcon />
          </IconButton>,
          <IconButton>
          ...
          <SwipeableTileList autoplay={true}>
            {centralNoticeList.slice(0, 2).map((item, index) => (
              <Tile
                ...
                onClick={() => router.push(`/${lng}/central-notices/${item.id}/` )}
              />
            ))}
          </SwipeableTileList>
        ) : (
