...
export const getServerSideProps = withAuthServerSideProps(async (context, selfUser) => {
  ...
  return {
    props: { selfUser, storeReview: storeReviewResponse.data, store: storeResponse.data },
  };
})

function Id({ selfUser, storeReview, store }) {
  const router = useRouter();
  return (
    <Layout title={`${storeReview.review.article.title} - ${process.env.NEXT_PUBLIC_APPLICATION_NAME}`}>
      <Section
        backButton
