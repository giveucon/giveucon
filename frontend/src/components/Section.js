...
export default function Section({ backButton=false, children=null, loading=false, padding=true, title=null, titlePrefix=null, titleSuffix=null }) {
      ...
      <Paper>
        {
          loading ? (
            <Box paddingX={1} paddingY={0.5}>
              <Box
                display={(backButton || titlePrefix || title || titleSuffix) ? 'flex' : 'none'}
                alignItems='center'
                flexDirection='row'
              ...
              <Box display={children ? 'block' : 'none'} marginX={0.5} marginY={1}>
